...
%   Author: Kamil Wojcicki, UTD, November 2011
    ...
    if isunix()
        ...
        scores = pesqbin_linux( reference, degraded, fs, mode, 'pesq.exe' );
    else 
...
function [ scores ] = pesqbin_linux( reference, degraded, fs, mode, binary )
    ...
    switch lower( mode )
        ...
        case { [], '', 'nb', '+nb', 'narrowband', '+narrowband' }
            ...
            command = sprintf( 'cd %s && wine %s +%i %s %s && cd -', tmpdir, binary, fs, file.reference, file.degraded );
        ...
        case { 'wb', '+wb', 'wideband', '+wideband' }
            ...
            command = sprintf( 'cd %s && wine %s +%i +wb %s %s && cd -', tmpdir, binary, fs, file.reference, file.degraded );
