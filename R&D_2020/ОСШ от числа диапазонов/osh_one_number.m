...
OSH_do = [];
OSH_posle = [];
N_onep = [1 2 3 4  8 9 10 14 16 20 25 27 32 40 49 64 75 100 128 250 500 1000 2000 4000 ];
for N_one = [1 2 3 4  8 9 10 14 16 20 25 27 32 40 49 64 75 100 128 250 500 1000 2000 4000 ]
...
OSH_do = [OSH_do OSH_one_exp_notfiltr];
OSH_one_exp_notfiltr = 0;
...
OSH_posle = [OSH_posle OSH_two_exp_filtr];
OSH_two_exp_filtr = 0;
end

N_onezzona =fix(length(sig)./(2*N_onep))-1;
kk = db(OSH_posle - OSH_do);
figure(1);
plot(N_onezzona,kk);
grid on;
ylabel('Выигрыш в ОСШ, дБ');
xlabel("Число диапазонов");